...
def newton_step(
    ...
    new_norm_node = GraphNode(name="norm", location=DataLocation.SERVER)
    ...
    class NewtonStep(MapReduceOperator):
        ...
        def map(
            ...
            norm_score = norm(G, ord=self.ord)
            # TODO send the norm_score to the contract
            def upload_norm_score(norm_score, node_address):
                if node_address is not None:
                    # upload the norm_score to the contract
                    print(f"upload norm_score {norm_score} to {node_address}")
            upload_norm_score(norm_score, self.node_address)
            return {
                "f": F,
                "score": G,
                "hessian": H,
                "n": np.asarray(obs),
                "norm_score": np.asarray(norm_score),
            }
        ...
        def reduce(
            ...
            params = params - d
            return params, f, iteration + 1, norm(d, ord=self.ord)
    ...
    new_norm_node.src = step_op
    return new_params_node, new_f_val_node, new_iteration_node, new_norm_node
...
def fit_newton(
    ...
    for _ in range(maxiter):
        params_node, f_val_node, iteration_node, norm_node = newton_step(
            f,
            score,
            hessian,
            x_node,
